...
function CommunityZone(){
    ...
    useEffect(() => {
        async function FetchCommunity() {
          ...
          const titleArray = response.data.map(item => ({ _id: item._id, title: item.title }));
    ...
    return(
        <div className="PostFrame">
                ...
                {currentItems.map((title, index) => (
                    <div className="postitem" key={index} >
                        <Link to={`/PostDetail/${index}`} style={{ display: "flex", alignItems: "center" }}>
                            <p className="post_index" style={{marginLeft:"1%"}}>{startIndex+index + 1}</p>
                            <p className="post_title" style={{marginLeft:"12%"}}>{title.title}</p>
            ...
            <div style={{display:"flex",justifyContent:"space-between"}}>
                ...
                {/* 페이지네이션 */}
                <div className="pagination" style={{justifyContent:"center",alignItems:"center",marginRight:"3%"}}>
