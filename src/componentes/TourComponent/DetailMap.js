...
function DetailMap({ gpsX, gpsY }) {
  ...
  useEffect(() => {
        ...
        content:
          ...
          `${
            row[columnIndices.HMPG_URL] === "0"
              ? `<a onclick="alert('현재 존재하는 웹사이트가 없습니다.')" style="cursor: not-allowed;"><span class="michelin-title">${
                  row[columnIndices.FCLTY_NM]
                }</span></a>`
              : `<a href="http://${
                  row[columnIndices.HMPG_URL]
                }" target="_blank"><span class="michelin-title">${
                  row[columnIndices.FCLTY_NM]
                }</span></a>`
          }` +
          `</div>`,
    ...
    // 여기서부터는 백승훈 추가내용.
    // 카카오맵 api categorySearch 메서드를 이용해서 음식점 데이터를 따로 쓰지 않고 지도에서 불러다 쓰는법 테스트중
